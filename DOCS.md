...
#### 일정 탭 - 다중 파일 첨부 기능
- 파일 input에 `multiple` 속성 추가 → 2개 이상 파일 동시 선택 가능
- 일정 수정 시 기존 첨부 파일 유지하며 새 파일 추가 가능 (덮어쓰기 방지)
- 첨부 파일을 JSON 배열로 DB 저장, 기존 단일 파일 형식과 하위 호환 처리
- 폼 내 파일 목록 표시 + 개별 파일 제거 버튼(✕) 추가
- DB 컬럼: `file_url`에 `[{"url":"...","name":"..."}]` JSON 형태로 저장

#### 일정 탭 - 행별 댓글 기능
- 각 일정 행에 💬 댓글 토글 버튼 추가 (클릭 전까지 댓글 패널 숨김)
- 댓글 항목 구성: 작성자(로그인 이름 자동 입력) + 내용 + 작성 일시
- 관리자 계정에서만 댓글 삭제 버튼(✕) 노출
- 한 번 로드된 댓글은 재요청 없이 재사용, 등록/삭제 시 재로드
- Supabase `schedule_comments` 테이블 신규 추가 및 연동

#### schedule_comments 테이블 스키마
| 컬럼 | 타입 | 설명 |
|------|------|------|
| id | UUID | 기본 키 |
| schedule_id | UUID | schedules 테이블 외래 키 (ON DELETE CASCADE) |
| author | TEXT | 작성자 이름 |
| content | TEXT | 댓글 내용 |
| created_at | TIMESTAMP | 작성 일시 |

#### 일정 탭 - 열 순서 및 너비 조정
- 댓글 버튼 열 위치: 회차 바로 다음(발제자 앞)으로 이동
- 발제자 열 너비 `6em` 고정 (한글 6자 기준)
